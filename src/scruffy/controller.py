...
from .state import ACTIVE_STATES, emit, job_from_spec, load_recovered_state, refresh_nodes
from .storage import (
    controller_lock,
    ensure_layout,
    find_request,
...
def _initialize_controller(
    *,
    ...
    for job in active:
        ...
        job["assignment"] = None
    for job in active:
        emit(controller, "job.lost", job=job)
...
def _ingest_requests(controller: Controller) -> None:
    ...
    for spec in specs:
        ...
        next_order += 1
        try:
            job = job_from_spec(spec, next_order)
        except Exception as exc:
            job = _rejected_job(spec, next_order, job_id, exc)
            known[job["id"]] = job
            emit(controller, "job.rejected", job=job)
            continue
        ...
        known[job_id] = job
        request = ResourceRequest.from_dict(job["request"])
        if request_can_ever_fit(controller.inventory, request):
