...
class DashboardTests(unittest.TestCase):
    ...
    def test_assets_and_read_only_api_are_served_on_loopback(self) -> None:
        ...
        self.assertIn(b'--mono:"IBM Plex Mono"', css)
        self.assertNotIn(b'Monaspace Xenon', css)
        ...
        self.assertIn(b"function jobOutput(job)", app)
        self.assertIn(b"function placementRows(job)", app)
        self.assertIn(b"Bounded, readable tails", app)
        self.assertNotIn(b'JSON.stringify(job.assignment', app)
        self.assertIn(b".job-output-button", css)
        ...
        self.assertIn(b'id="gpu-job-open"', html)
        self.assertIn(b"function assignedJobSummary(jobId)", app)
        self.assertIn(b"View job & output", app)
